...
const ArmindoAccount = (function () {
    ...
    function init() {
        ...
        try {
            ...
            // Handle redirect result (for COOP mitigation)
    ...
    /**
     * Internal handler for signInWithRedirect results
     */
    async function _handleRedirectResult() {
        try {
            const result = await _auth.getRedirectResult();
            if (result && result.user) {
                console.log('✅ Login via redirecionamento concluído para:', result.user.email);
    ...
    /**
     * Sign in with Google
     * @param {string} method - 'popup' (default) or 'redirect'
     */
    ...
    function _getErrorMessage(code) {
        const messages = {
            ...
            'auth/requires-recent-login': 'Esta ação requer que tenha feito login recentemente. Volte a entrar na conta e tente de novo.'
        };
